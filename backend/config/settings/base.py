...
# Hours after which unverified tenants (and their owner users) are cleaned up
UNVERIFIED_TENANT_CLEANUP_HOURS = 48
...
# Django's own CSRF origin check — must include every origin that POSTs to the API.
# In production, add your frontend domain via the CSRF_TRUSTED_ORIGINS env var.
CSRF_TRUSTED_ORIGINS = config(
    "CSRF_TRUSTED_ORIGINS",
    default=f"http://localhost:3000,http://127.0.0.1:3000,https://{BASE_DOMAIN},https://*.{BASE_DOMAIN}",
    cast=Csv(),
)
CORS_ALLOW_HEADERS = [
    "accept",
...
CELERY_RESULT_EXPIRES = 3600  # 1 hour — prevent unbounded Redis growth
...
if USE_S3:
    ...
    MEDIA_URL = "/"  # not used; files are served via pre-signed URLs generated by boto3
else:
    MEDIA_URL = "/media/"
