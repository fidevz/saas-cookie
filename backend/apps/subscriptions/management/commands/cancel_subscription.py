...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "email", help="Email of the tenant owner whose subscription to cancel."
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        if not owned_tenants.exists():
            ...
            if memberships.exists():
                ...
                raise CommandError(
                    ...
                    "Re-run the command with the owner's email to cancel the subscription."
                )
            raise CommandError(
                f"{email} is not the owner of any tenant and has no memberships."
            )
        ...
        self.stdout.write(
            f"Stripe sub:   {subscription.stripe_subscription_id or '(none)'}"
        )

        if dry_run:
            if subscription.stripe_subscription_id:
                self.stdout.write(
                    f"  Would cancel Stripe subscription: {subscription.stripe_subscription_id}"
                )
            self.stdout.write(
                f"  Would delete local Subscription record (pk={subscription.pk})"
            )
            return
        ...
        if subscription.stripe_subscription_id:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"  ✓ Cancelled in Stripe: {subscription.stripe_subscription_id}"
                    )
                )
            ...
            self.stdout.write(
                "  · No Stripe subscription ID — skipping Stripe cancellation."
            )

        # ── 5. Delete local record ────────────────────────────────────────────
        subscription.delete()
        self.stdout.write(
            self.style.SUCCESS("  ✓ Subscription record deleted from database.")
        )
